...
/**
 * Парсер новостей из RSS ленты orengrad.ru
 *
 */
class Sia extends TyRunBaseParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const MAIN_PAGE_URI = 'https://sia.ru';
     ...
     */
    const FEED_URL = 'https://sia.ru/RSS/sia-news-rss.php';
    ...
    public static function run(): array
    {
        ...
        $crawler->filter('rss channel item')->slice(0, self::MAX_NEWS_COUNT)->each(function (Crawler $node) use (&$curl, &$posts) {
            ...
            if (!empty($newsContent)) {
                 ...
                 */
                $articleContent = $newsContent->filter('.clearfix')->children();
                $stopParsing = false;
    ...
    protected static function parseNode(Crawler $node, NewsPost $newPost, int $maxDepth, bool &$stopParsing, $descriptionSentences = []): void
    {
        ...
        switch ($node->nodeName()) {
            ...
            case 'p':
            case 'span':
                if ($node->filter(self::QUOTE_TAG)->count()) {
                    self::parseDescriptionIntersectParagraph($node->filter(self::QUOTE_TAG)->first(), $newPost, $descriptionSentences);
                } else {
