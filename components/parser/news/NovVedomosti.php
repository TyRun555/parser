...
/**
 * Парсер новостей из RSS ленты novvedomosti.ru
 *
 */
class NovVedomosti extends TyRunBaseParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const MAIN_PAGE_URI = 'https://novvedomosti.ru';

    /**
     * CSS класс, где хранится содержимое новости
     */
    const BODY_CONTAINER_CSS_SELECTOR = '.col_left';
     ...
     */
    const FEED_URL = 'https://novvedomosti.ru/news.xml';
    ...
    public static function run(): array
    {
        ...
        $crawler->filter('rss channel item')->slice(0, self::MAX_NEWS_COUNT)->each(function (Crawler $node) use (&$curl, &$posts) {
            ...
            if (!empty($newsContent)) {
                 ...
                 */
                $articleContent = $newsContent->filter('.text')->children();
                $stopParsing = false;
